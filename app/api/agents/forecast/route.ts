...
const FORECAST_AGENT_ID = "69272b00cebc7452a28f8d9d";
const FORECAST_API_KEY = process.env.FORECAST_AGENT_API_KEY || "YOUR_PORTFOLIO_AGENT_API_KEY_HERE"; 
  ...
  try {
    ...
    const res = await fetch(NEW_AGENT_URL, {
      ...
      body: JSON.stringify({
        ...
        agent_id: FORECAST_AGENT_ID,
        // 3. Use agent ID to ensure session is unique per agent
        session_id: `${FORECAST_AGENT_ID}-${SESSION_ID}`, 
