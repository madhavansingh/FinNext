...
const SENTIMENT_AGENT_ID = "692726db642e89081dd9da52";
const SENTIMENT_API_KEY = process.env.SENTIMENT_AGENT_API_KEY || "YOUR_PORTFOLIO_AGENT_API_KEY_HERE"; 
  ...
  try {
    ...
    const res = await fetch(NEW_AGENT_URL, {
      ...
      body: JSON.stringify({
        ...
        agent_id: SENTIMENT_AGENT_ID,
        // 3. Use agent ID to ensure session is unique per agent
        session_id: `${SENTIMENT_AGENT_ID}-${SESSION_ID}`, 
